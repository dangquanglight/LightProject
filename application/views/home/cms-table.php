<?php 		
	$paging_bar = paging_bar($data['num_rows'], $data['page_size'], $data['page'], $controller_name, "ajax_cms", get_url_navigate()); ?>
	<?php echo $paging_bar; ?>
<table cellpadding="0" cellspacing="0" width="100%" class="table-list" page_size="<?php echo $data['page_size'];?>">
	<thead>
		<tr>
			...
			<th style="width:120px">
				<a href='javascript:void(0)' sort='title-kr'><?php echo $this->lang->line('title-kr'); ?></a>
			</th>
			<th style="width:80px">
	...
	<tbody>
		...
		if ($total > 0):
			...
			<tr <?php echo $i % 2 == 0 ? "class='even'" : "";?>>
				...
				<td>
					<?php echo htmlspecialchars($row->title_kr); ?>
				</td>
				<td class="tc">
				...
				<td class="tc">					
					<a href="<?php echo base_url("edit-cms/" . $row->id . "?" . get_url_navigate());?>"><?php echo $this->lang->line('edit');?></a> 
					| <a href="javascript:void(0)" onclick="delete_1_data('cms', 'delete_cms', '<?php echo $row->id; ?>')"><?php echo $this->lang->line('delete');?></a>					
				</td>
			</tr>
			<?php
			$start_index++;
