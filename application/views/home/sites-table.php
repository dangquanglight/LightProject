<?php 		
	$paging_bar = paging_bar($data['num_rows'], $data['page_size'], $data['page'], $controller_name, "ajax_sites", get_url_navigate()); ?>
	<?php echo $paging_bar; ?>
<table cellpadding="0" cellspacing="0" width="100%" class="table-list" page_size="<?php echo $data['page_size'];?>">
	<thead>
		<tr>
			...
			<th>
				<a href='javascript:void(0)' sort="site-name"><?php echo $this->lang->line('site-name');?></a>				
			</th>
			<th style="width:250px">
				<a href='javascript:void(0)'><?php echo $this->lang->line('description');?></a>
			</th>
			<th style="width:50px">
				<a href='javascript:void(0)'><?php echo $this->lang->line('zones');?></a>
			</th>
	...
	<tbody>
		...
		if ($total > 0):
			...
			<tr <?php echo $i % 2 == 0 ? "class='even'" : "";?>>
				...
				<td>					
					<a href='<?php echo base_url('edit-site/' . $row->id);?>'><?php echo htmlspecialchars($row->site_name); ?></a>
				</td>
				<td>
					<?php echo htmlspecialchars($row->description); ?>
				</td>
				<td class='tc'>					
					<a href='<?php echo base_url('zones?site_id=' . $row->id);?>'>[ <?php echo htmlspecialchars($row->total_zones); ?> ]</a>
				</td>
				<td>
				...
				<td class="tc">					
					<a href="<?php echo base_url("edit-site/" . $row->id . "?" . get_url_navigate()); ?>"><?php echo $this->lang->line('edit');?></a> 
					| <a href="javascript:void(0)" onclick="delete_1_data('<?php echo $this->lang->line('site');?>', 'delete_sites', '<?php echo $row->id; ?>')"><?php echo $this->lang->line('delete');?></a>					
				</td>
			</tr>
			<?php
			$start_index++;
