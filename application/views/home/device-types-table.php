<?php 		
	$paging_bar = paging_bar($data['num_rows'], $data['page_size'], $data['page'], $controller_name, "ajax_device_types", get_url_navigate()); ?>
	<?php echo $paging_bar; ?>
<table cellpadding="0" cellspacing="0" width="100%" class="table-list" page_size="<?php echo $data['page_size'];?>">
	<thead>
		<tr>
			...
			<th style="width:50px">
				<a href='javascript:void(0)'><?php echo $this->lang->line('devices');?></a>
			</th>
	...
	<tbody>
		...
		if ($total > 0):
			...
			<tr <?php echo $i % 2 == 0 ? "class='even'" : "";?>>
				...
				<td>					
					<a href='<?php echo base_url('edit-device-type/' . $row->id);?>'><?php echo htmlspecialchars($row->type_name); ?></a>
				</td>
				<td>
					<?php echo htmlspecialchars($row->description); ?>
				</td>
				<td class='tc'>					
					<a href='<?php echo base_url('devices?type_id=' . $row->id);?>'>[ <?php echo htmlspecialchars($row->total_devices); ?> ]</a>
				</td>
				<td>
				...
				<td class="tc">					
					<a href="<?php echo base_url("edit-device-type/" . $row->id . "?" . get_url_navigate()); ?>"><?php echo $this->lang->line('edit');?></a> 
					| <a href="javascript:void(0)" onclick="delete_1_data('<?php echo $this->lang->line('device_type');?>', 'delete_device_types', '<?php echo $row->id; ?>')"><?php echo $this->lang->line('delete');?></a>					
				</td>
			</tr>
			<?php
			$start_index++;
