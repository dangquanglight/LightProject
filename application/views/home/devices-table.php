<?php 		
	$paging_bar = paging_bar($data['num_rows'], $data['page_size'], $data['page'], $controller_name, "ajax_devices", get_url_navigate()); ?>
	<?php echo $paging_bar; ?>
	...
	<tbody>
		...
		if ($total > 0):
			...
			<tr <?php echo $i % 2 == 0 ? "class='even'" : "";?>>
				...
				<td>
					<a href="<?php echo base_url('edit-site/' . htmlspecialchars($row->site_id)); ?>"><?php echo htmlspecialchars($row->site_name); ?></a>
				</td>
				<td>
				...
				<td class="tc">					
					<a href="<?php echo base_url("edit-device/" . $row->id . "?" . get_url_navigate()); ?>"><?php echo $this->lang->line('edit');?></a> 
					| <a href="javascript:void(0)" onclick="delete_1_data('<?php echo $this->lang->line('device');?>', 'delete_devices', '<?php echo $row->id; ?>')"><?php echo $this->lang->line('delete');?></a>					
				</td>
			</tr>
			<?php
			$start_index++;
