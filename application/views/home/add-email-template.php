...
<h1 class="form-title"><?php echo $this->lang->line('email-template'); ?></h1>
<?php if ($data['data']){ ?>
	<?php echo show_saved_data($data['result'], $data['err_msg']); ?>
	<form id="form-data" class='f-email-template' method="post" enctype="multipart/form-data">	
		...
		<div id='china' style='<?php echo !$data['ci_open_status'] ? 'display:none' : '';?>'>
			<div class="ib">
				<label class="text" for="title_ci"><?php echo $this->lang->line('title-ci'); ?></label>
				<input type="textbox" id="title_ci" name="title_ci" class="normal" value="<?php echo $data['data']->title_ci; ?>"/>		
			</div>
			<div class="ib">			
				<textarea id="content_ci" name="content_ci"><?php echo htmlspecialchars($data['data']->content_ci); ?></textarea>
				<script type="text/javascript">
					CKEDITOR.replace( 'content_ci');
				</script>
			</div>			
...
<?php } else { ?>
	<div class="fail"><?php echo get_not_found_long('email-template', $this->lang->line('email-template'));?></div>
<?php } ?>