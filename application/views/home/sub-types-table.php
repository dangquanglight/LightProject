<?php 		
	$paging_bar = paging_bar($data['num_rows'], $data['page_size'], $data['page'], $controller_name, "ajax_sub-types", get_url_navigate()); ?>
	<?php echo $paging_bar; ?>
<table cellpadding="0" cellspacing="0" width="100%" class="table-list" page_size="<?php echo $data['page_size'];?>">
	<thead>
		<tr>
			...
			<th>
				<a href='javascript:void(0)' sort="sub-type-name"><?php echo $this->lang->line('sub-type-name');?></a>				
			</th>
	...
	<tbody>
		...
		if ($total > 0):
			...
			<tr <?php echo $i % 2 == 0 ? "class='even'" : "";?>>
				...
				<td>					
					<a href='<?php echo base_url('edit-sub-type/' . $row->id);?>'><?php echo htmlspecialchars($row->sub_type_name); ?></a>
				</td>
				<td>
					<a href='<?php echo base_url('edit-type/' . $row->type_id);?>'><?php echo htmlspecialchars($row->type_name); ?></a>
				</td>
				<td>
				...
				<td class="tc">					
					<a href="<?php echo base_url("edit-sub-type/" . $row->id . "?" . get_url_navigate()); ?>"><?php echo $this->lang->line('edit');?></a> 
					| <a href="javascript:void(0)" onclick="delete_1_data('<?php echo $this->lang->line('sub-type');?>', 'delete_sub_types', '<?php echo $row->id; ?>')"><?php echo $this->lang->line('delete');?></a>					
				</td>
			</tr>
			<?php
			$start_index++;
